...
class Network:
    def __init__(self, dataset, config):
        ...
        self.adaboost_clf = AdaBoostClassifier(DecisionTreeClassifier(max_depth=3), n_estimators=50, learning_rate=1.0)
    ...
    def train(self, dataset):
        ...
        while self.training_epoch < self.config.max_epoch:
            ...
            try:
                ...
                # Train AdaBoost classifier on logits and labels
                self.adaboost_clf.fit(probs, labels)
    ...
    def evaluate(self, dataset):
        ...
        for step_id in range(self.config.val_steps):
            try:
                ...
                pred = np.argmax(stacked_prob, axis=1)

                # Handle ignored labels if applicable
                if self.config.ignored_label_inds:
                    invalid_idx = np.where(labels == self.config.ignored_label_inds)[0]
                    labels_valid = np.delete(labels, invalid_idx)
                    labels_valid = labels_valid - 1  # adjust label values if needed
        ...
        for n in range(self.config.num_classes):
            ...
            iou = true_positive_classes[n] / denominator if denominator > 0 else 0
            iou_list.append(iou)

        # --- Weighted IoU Calculation ---
        # Provided class distribution (0-indexed: class 0 corresponds to original class 1, etc.)
        distribution_each_class = {0: 22.56, 1: 13.37, 2: 15.56, 3: 4.55, 4: 38.04, 5: 3.76, 6: 0.7, 7: 1.47}
        # Inverse-proportional weights
        weights = {k: 1.0 / v for k, v in distribution_each_class.items()}
        total_weight = sum(weights.values())
        normalized_weights = {k: v / total_weight for k, v in weights.items()}
        weighted_iou = sum(iou_list[n] * normalized_weights[n] for n in range(self.config.num_classes))
        # -------------------------------

        log_out('Weighted IoU: {:.1f}%'.format(100 * weighted_iou), self.Log_file)
        return weighted_iou
